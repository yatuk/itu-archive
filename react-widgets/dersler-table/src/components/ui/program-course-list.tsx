...
export function ProgramCourseList({ items, labels, onOpen, onCopy, onOpenObs, onRemove, onReorder }: ProgramCourseListProps) {
  ...
  if (!items.length) return <div className="pcl-empty"><BookOpen aria-hidden="true" /><strong>{labels.empty}</strong></div>;
  const drop = (event: DragEvent, to: number) => { event.preventDefault(); if (drag !== null && drag !== to) onReorder(drag, to); setDrag(null); };
  return <div className="pcl-list" role="table">
    <div className="pcl-head" role="row"><span role="columnheader">{labels.course}</span><span role="columnheader">{labels.quota}</span></div>
    {items.map((item, index) => <article
      key={item.key}
      className={`pcl-item${item.full ? ' is-full' : ''}`}
      role="row"
      draggable
    ...
    >
      ...
      <div className="pcl-course" role="cell">
        <div className="pcl-title"><strong>{item.code}</strong>{item.badge && <span>{item.badge}</span>}</div>
        <p>{item.name}</p>
        <div className="pcl-meta">
          <span><Clock3 aria-hidden="true" />{item.when}</span>
          <span><UserRound aria-hidden="true" />{item.instructor}</span>
        </div>
        {item.credit && <small>{item.credit}</small>}
      </div>
      <div className="pcl-numbers" role="cell"><b>{item.quota}</b><span>CRN {item.crn}</span>{item.backup && <em>{item.backup}</em>}</div>
      <button className="pcl-remove" type="button" onClick={() => onRemove(item.key)} aria-label={`${item.code} ${labels.remove}`}><Trash2 aria-hidden="true" /></button>
      <div className="pcl-menu-wrap">
        <button className="pcl-menu-trigger" type="button" aria-expanded={menu === item.key} aria-haspopup="menu" onClick={() => setMenu(menu === item.key ? null : item.key)}><MoreHorizontal aria-hidden="true" /><span className="sr-only">{item.code} {labels.actions}</span></button>
        {menu === item.key && <div className="pcl-menu" role="menu">
          <button role="menuitem" onClick={() => { onOpen(item.key); setMenu(null); }}><BookOpen />{labels.details}</button>
          <button role="menuitem" onClick={() => { onCopy(item.key, 'crn'); setMenu(null); }}><Copy />{labels.copyCrn}</button>
          <button role="menuitem" onClick={() => { onCopy(item.key, 'code'); setMenu(null); }}><Copy />{labels.copyCode}</button>
          {item.instructor && <button role="menuitem" onClick={() => { onCopy(item.key, 'instructor'); setMenu(null); }}><UserRound />{labels.copyInstructor}</button>}
